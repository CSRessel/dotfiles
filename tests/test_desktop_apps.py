        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def merge(value):
                return subprocess.check_output(
                    ["python3", str(ROOT / "dot_config/modify_mimeapps.list")],
                    input=value,
                    text=True,
                    env=env,
                )
            ...
            self.assertEqual(
                defaults["x-scheme-handler/terminal"], "com.mitchellh.ghostty.desktop;"
            )
            self.assertEqual(
                defaults["application/x-terminal-emulator"], "com.mitchellh.ghostty.desktop;"
            )
