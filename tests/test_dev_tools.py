            ...
            for enabled in [False, True]:
                ...
                for name, shell in [(".bashrc", "bash"), (".zshrc", "zsh")]:
                    rendered = subprocess.check_output(
                        base + ["cat", str(home / name)], env=env, text=True
                    )
    ...
    def test_paths_are_idempotent_and_preserve_nix(self):
        for shell in ["bash", "zsh"]:
            ...
            for nix in [False, True]:
                env = dict(
                    os.environ, HOME="/tmp/dev-tools-test", PATH="/nix/test/bin:/usr/bin:/bin"
                )
                ...
                result = subprocess.check_output(
                    [
                        ...
                        '. "$1"; . "$1"; printf "%s" "$PATH"',
                        "test",
                        str(ROOT / ".chezmoitemplates/dev-tools-env"),
                    ],
                    env=env,
                    text=True,
                )
