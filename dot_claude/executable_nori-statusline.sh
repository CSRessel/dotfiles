...
# Build status line: ⎇ main · 7% / 1M
# just yellow branch, and gray context window pct/size
PCT=$(echo "$INPUT" | jq -r '.context_window.used_percentage // 0')
SIZE_RAW=$(echo "$INPUT" | jq -r '.context_window.context_window_size // 200000')
SIZE_FORMATTED=$(format_tokens "$SIZE_RAW" | sed 's/\.0\([kM]\)/\1/')
PCT_FORMATTED=$(printf "%.0f" "$PCT" 2>/dev/null || echo "$PCT")

echo -e "${YELLOW}⎇ ${BRANCH}${NC} ${DIM_WHITE}· ${PCT_FORMATTED}% / ${SIZE_FORMATTED}${NC}"
